...
export default function ShareFicheButton({
  ...
  const ogTitle = `${nomAffiche}${metier ? `  ${metier}` : ""}`;
  const ogDescription = `Artisan${commune ? ` à ${commune}` : ""} sur Oyez Artisans ! Consultez sa fiche et contactez-le directement.`;
  const ogDomain = "oyezartisans.fr";
  ...
  return (
    <div className="mt-3 space-y-3">
      ...
      <div>
        ...
        >
          {previewVisible ? "Masquer l'aperçu" : " Voir ce que verront tes contacts"}
        </button>
          ...
          <div className="mt-2 max-w-sm">
            <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
              <div className="relative aspect-[1200/630] w-full bg-gray-100">
                <Image
                  src={carteSrc}
                  alt={`Aperçu partage de ${nomAffiche}`}
                  fill
              ...
              <div className="border-t border-gray-100 px-3 py-2">
                <p className="truncate text-xs uppercase tracking-wide text-gray-400">{ogDomain}</p>
                <p className="mt-0.5 line-clamp-2 text-sm font-semibold text-gray-900">{ogTitle}</p>
                <p className="mt-0.5 line-clamp-2 text-xs text-gray-500">{ogDescription}</p>
              </div>
            </div>
            <p className="mt-1.5 text-xs text-gray-400">
              C&apos;est ce qui s&apos;affiche quand tu partages ton lien sur Facebook, LinkedIn, WhatsApp, X
            </p>
          </div>
        )}
      </div>
