...
/**
 * Email de bienvenue pour les nouveaux artisans connectés via Google OAuth.
 * Leur compte est déjà VALIDE — on les guide à compléter leur fiche pour apparaître dans l'annuaire.
 */
export async function sendBienvenueArtisanGoogle({
  destinataireEmail,
  prenomArtisan,
}: {
  destinataireEmail: string;
  ...
  try {
    await resend.emails.send({
      from: `OyezArtisans <${FROM}>`,
      to: destinataireEmail,
      subject: `🔨 Bienvenue sur Oyez Artisans ! Complétez votre fiche pour être visible`,
      html: `
<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#fff8f0;font-family:sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#fff8f0;padding:32px 16px;">
          ...
          <td style="padding:28px;">
            ...
            <div style="background:#fff8f0;border-left:4px solid #6bcb77;border-radius:4px;padding:14px 16px;margin-bottom:24px;">
              <p style="margin:0;font-size:14px;color:#333;">
                <strong>📍 Pour apparaître dans l&apos;annuaire</strong>, rendez-vous dans votre espace et renseignez votre fiche :
              </p>
              <ul style="margin:8px 0 0 0;padding-left:20px;font-size:14px;color:#555;">
                <li>Votre métier (maçon, plombier, électricien…)</li>
                <li>Votre commune d&apos;intervention</li>
              </ul>
              <p style="margin:8px 0 0;font-size:14px;color:#333;">→ Votre fiche sera <strong>en ligne immédiatement</strong> une fois ces informations saisies.</p>
            </div>
            <a href="${lienEspace}" style="display:inline-block;background:#ffd93d;color:#1a1a2e;font-weight:900;font-size:15px;padding:14px 28px;border-radius:8px;border:2px solid #1a1a2e;text-decoration:none;box-shadow:3px 3px 0 #1a1a2e;">
              Compléter ma fiche →
            </a>
            <p style="margin:20px 0 0;font-size:13px;color:#999;">
              Vous pourrez aussi ajouter une description, un logo et des photos de vos réalisations pour vous démarquer.
            </p>
          </td>
        </tr>
        <tr>
    ...
    console.error("[email] Échec email bienvenue Google artisan:", err);
  }
}
