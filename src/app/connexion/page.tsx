...
function ConnexionForm() {
  ...
  async function handleEmailNext(e: React.FormEvent) {
    ...
    const data = (await res.json()) as { exists: boolean; googleOnly?: boolean };
    setLoading(false);
  ...
  return (
    ...
    >
      ...
      {step === "password" && googleOnly && (
        <div className="flex flex-col gap-3">
          <p className="rounded-lg bg-[#60c5f1]/20 px-4 py-3 text-sm font-semibold text-[#1a1a2e]">
            🔵 Cet email utilise la connexion Google.
            <br />
            <span className="text-xs font-normal text-[#1a1a2e]/70">
          ...
          <button
            type="button"
            onClick={() => signIn("google", { callbackUrl })}
            className="flex w-full items-center justify-center gap-3 rounded-xl border-2 border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50"
          >
            <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
              <path
                fill="#4285F4"
