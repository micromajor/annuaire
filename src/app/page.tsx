...
export default async function HomePage() {
  ...
  const isArtisan = (session?.user as { role?: string })?.role === "artisan";
  const artisanId = (session?.user as { id?: string })?.id;

  // Si artisan connecté : charger son prénom + vérifier que le compte existe
  ...
  return (
    <div className={`flex min-h-screen flex-col ${isArtisan ? "bg-[#6bcb77]" : "bg-[#ffd93d]"}`}>
      {/* Header minimaliste */}
